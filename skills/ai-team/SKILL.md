...
角色之间**直接互发消息**协作。**PM 不中转、不干预**正常协作，只在**队列放行**（§3.2）与**收敛裁决**（§3.4）两处介入。

> 沟通约定（消息只传指针）与台账要求是**面向角色的**约束，见 `ai-team-tool-global-rule` §十一；PM 无需代为转达业务内容。

### 3.2 审查队列与闭环门禁

**[门禁] 一次只放行一个审查维度**（队列串行）：顺序按「发现问题的破坏力降序」排定，前一个**闭环完成**后才 spawn / 放行下一个（机制见 `global-rule` §十一 11.1）。

`手动确认` 模式下，在**首次交付完成的弹窗**里追加一问，**问题文案须点名本次待放行的维度清单**（例：「是否并行执行多个审查维度？（将同时拉入：{维度A}、{维度B}；并行执行审查）」），默认**否**；用户选是 → 并行组须满足 4 条准入（§十一 11.1），其中「期间不得驱动产出方返工」为硬约束。

**[门禁] 放行前断言**（§十一 11.2）：① 前序报告存在且**状态为「定稿」**；② 前序 **`未闭环问题数 = 0`**；③ `depends_on` 声明的报告**存在且已定稿**。不满足 → **不放行**，要求产出方闭环后重报。
...
> **模式联动**：全自动模式收到完成信号即按上表放行；手动确认模式下，完成度确认已在角色会话内完成，PM 收到信号后**不再重复向用户确认**。
>
> **报告质量由角色自管**：产出角色置信度不足会自行迭代至达标；下游发现问题**直接与产出角色沟通**（跨角色问题须登记台账）。PM 无需代为审阅报告。
>
...
> 触发条件与**角色侧动作**（提出方直接问用户）见 `global-rule` §十一 11.4；本节只写 **PM 自己要做什么**。

| PM 收到 | 动作 |
|---------|------|
| 角色报「无法达成共识 / 需用户裁决」 | 汇总「问题 / 双方观点 / 已尝试 / **推荐方案 + 理由**」→ `ask_followup_question` 统一提问用户 |
| 用户裁定「继续（计数重置为 0）」 | 恢复放行该问题相关维度 |
| 用户裁定「就此收口交付」 | **停止放行受影响的审查维度**，直接进入收尾；收尾报告标注「按用户裁定提前收口」+ 遗留 issue 列表 |
| 用户裁定「终止任务」 | 停止编排 → 走阶段 4 收尾与会话回收 |
