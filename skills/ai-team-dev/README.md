...
| 开发 | 读需求 → 查平台映射 → 编码 + 编译验证；多任务时阶段式开发、逐任务汇报 | 源码 + `coder-report.md`（多任务含 `coder-report-task-{N}.md`） |
| 测试 | 读需求 → 查平台映射 → 测试 + 缺陷反馈 | `tester-report.md` |
...
flowchart TD
    ...
    C --> D["阶段 2 · 开发 Agent<br/>读需求文档 → 查平台映射 → 技术选型 → 编码 → 编译验证 → 产出交付报告<br/>置信度 92% ✅"]
...
| 跨角色沟通 | send_message 内存通道 | 会话结束销毁 |
