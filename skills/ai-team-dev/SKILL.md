...
>
> **[门禁] 放行前断言**：① 前序报告**状态为「定稿」**；② 前序 **`未闭环问题数 = 0`**；③ `depends_on` 声明的报告存在且已定稿。不满足 → **不放行**。
>
...
**[门禁] 一次只放行一个审查维度**：dev 恒为 `tester` → `reviewer`（`role-registry.md` 已声明该依赖），前一个**闭环完成**后才 spawn 下一个（机制见 `global-rule` §十一 11.1）。

`手动确认` 模式下，**coder 首次交付完成**的弹窗里追加一问「是否并行执行多个审查维度？」（默认**否**）；选是 → 并行组须满足 `global-rule` §十一 11.1 的 4 条准入（**期间不得驱动 coder 返工**为硬约束）。

**[门禁] 放行前断言（PM 侧）**：前序 `未闭环问题数 = 0`（该字段由角色完成信号携带，定义见 `global-rule` §十一 11.2）；tester 因「单测不可用 / 用户跳过」收口时，**若已报缺陷未闭环，同样不得放行**。

**收敛后的 PM 动作**（触发条件与角色侧动作见 `global-rule` §十一 11.4）：

| PM 收到 | 动作 |
|---------|------|
| 用户裁定「继续（计数重置为 0）」 | 恢复放行该问题相关维度 |
| 用户裁定「就此收口交付」 | **停止放行受影响维度**，直接收尾；报告标注「按用户裁定提前收口」+ 遗留 issue 列表 |
| 用户裁定「终止任务」 | 停止编排 → 走阶段 3 收尾与会话回收 |

> **仅限「角色间冲突 + 台账超限」**：角色对用户的**直接需求疑问**由角色自行提问，不绕 PM。
> **「受阻」不属于本节**——角色门禁失败报「受阻」按 2.1 的 `global-rule` §七 指针处置。
