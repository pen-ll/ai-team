---
name: ai-team-dev-pt-hm-project-module-init
autoTrigger: false
trigger: route-only
...
模块注册完成后，调用 `use_skill ai-team-dev-pt-hm-project-package-init`，让用户选择预装的 OHPM 包。

### 5. 验证编译

**[门禁] 必须显式 `use_skill ai-team-dev-pt-hm-build`**，按其中探测的路径与编译命令验证模块可编译（BUILD SUCCESSFUL），禁止自行内联编译命令。
