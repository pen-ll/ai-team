...
| 通用步骤 | 鸿蒙特化行为 |
|----------|-------------|
| 第二步：编译验证 | 替换为 `ai-team-pt-hm-build` 编译 |
...
审查阶段如需重新编译：**[门禁] 必须显式 `use_skill ai-team-pt-hm-build`**（编译命令、DEVECO_SDK_HOME 路径探测均由 build skill 提供，禁止自行猜测/内联）：
...
#### 安全审查（遵循 ai-team-pt-hm-arkts-security）
...
#### 性能审查（遵循 ai-team-pt-hm-arkts-performance）
