...
```
标题: "技术选型"
选项: ["由AI决定（AI 基于需求文档自动推断，无需逐项询问）", "用户介入（逐项选择状态管理/数据存储/编码模板）"]
```
...
- 选择"官方V2状态Demo" → `use_skill ai-team-dev-pt-hm-template-v2`
...
**[门禁] MCP LSP 校验通过后，必须显式 `use_skill ai-team-dev-pt-hm-build` 进行编译验证**（编译命令、DEVECO_SDK_HOME 路径探测均由 build skill 提供，禁止自行猜测/内联）：
