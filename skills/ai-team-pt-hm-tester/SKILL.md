...
| 第二步：测试流程（单测结束后） | **[门禁] 询问用户是否追加真机 UI 自动化测试**；选执行则加载 `ai-team-pt-hm-ui-test`，并在单测不可用时作为替代验证 |
| 第三步：产出文档 | 补充测试结果格式 |
...
生成或修改较完整的功能模块时，**必须**同步编写 ohosTest 单元测试，覆盖 Model、ViewModel、工具方法等非 UI 逻辑。
...
4. 编译主项目：**[门禁] 必须显式 `use_skill ai-team-pt-hm-build`**（编译命令、DEVECO_SDK_HOME 路径探测均由 build skill 提供，禁止自行猜测/内联），确保 BUILD SUCCESSFUL
...
8. 测试失败时先区分问题归属（同通用 tester「跨角色沟通协议」）：
   ...
   - **产品代码缺陷** → 向 coder-agent 反馈，等待修复后重新测试，不自行修改产品代码
...
10. **[门禁] 询问真机 UI 自动化测试**（单测执行结束后执行，含「单测因架构/环境限制无法执行」的情况，不可跳过）：`ask_followup_question` 询问「是否追加真机 UI 自动化测试？」——选项 `["执行真机 UI 自动化测试", "跳过，直接交付"]`（末位自定义项由 `ai-team-tool-global-rule` 统一追加）
    - 用户选执行 → `use_skill ai-team-pt-hm-ui-test`，按其流程执行，并把「运行时真机验证」章节写入 `tester-report.md`
...
> 通用门禁见 `ai-team-role-tester` 第二步「依赖用户操作的环境条件」；测试聚焦与用例数参考同通用（优先 ViewModel，跳过 UI 组件与设备 API 工具类）。
...
ui_test_skill: ai-team-pt-hm-ui-test   # 单测结束后询问用户，选执行时加载
```
