...
[![Version](https://img.shields.io/badge/Version-2.3-green)](#14-版本历史)
...
| 版本 | 范围 | 主题 | 关键变化 |
|------|------|------|----------|
| **2.3** | 2026-09-23 起 | **鸿蒙工具链口径纠正：统一到官方 `@deveco/deveco-cli`** | **包口径反转**：E2 由「首选测试通道包、禁用官方包」改为**官方包唯一**（裸包名 = 官方最新版），`@deveco-test/*` 与 `hmos-deveco-cli` 列为非官方通道；**迁移式安装**：bin 归属探测 → 卸载旧包 → `npm install -g`（安装即升级、幂等）→ 能力自检，覆盖同 bin 包 `EEXIST`；**入口形态**：`cli.js` 为启动器（逻辑在同级 `-common`），解析路径 / MCP 模板 / 新增 impl 缺失分类同步；**禁止 `devecocli update`**（目标为 bin 内嵌包名 + tag）；**新增门禁**「能力不足时不得自行换包 / 装包」；E2.2 超时归因改为「先能力自检再判定」并补工具链基线 |
